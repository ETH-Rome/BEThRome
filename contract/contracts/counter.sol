...
contract Lottery {
  ...
  function buyTicket(
    ...
    rewards[msg.sender] = FHE.select(
      isWinner,
      rewards[msg.sender].add(currentPrize),
      rewards[msg.sender]
    );
  ...
  modifier onlyBeforeEnd() {
    if (block.timestamp >= endingTime)
      revert TooLate(endingTime, block.timestamp);
    _;
  }

  modifier onlyAfterEnd() {
    if (block.timestamp < endingTime)
      revert TooEarly(endingTime, block.timestamp);
    _;
  }
}
